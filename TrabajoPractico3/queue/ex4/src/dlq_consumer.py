...
def setup_logger(name="dlq-consumer"):
    ...
    log_file = os.getenv("LOG_FILE", "/var/log/ex4/dlq_consumer.log")
    try:
...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        logger.error(
            "Mensaje recibido desde DLQ tras %s reintentos: %s",
            headers.get("x-retry-attempt", "desconocidos"),
            body.decode("utf-8"),
        )
    ...
    logger.info("Esperando mensajes fallidos en %s...", config["dead_letter_queue"])
