...
def setup_logger(name="main-consumer"):
    ...
    log_file = os.getenv("LOG_FILE", "/var/log/ex3/consumer.log")
    try:
...
def should_reject(message):
    return message.get("error") is True
...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        if should_reject(message):
            logger.warning("Mensaje rechazado y enviado a DLQ: %s", message)
            ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
            return

        logger.info("Mensaje procesado correctamente: %s", message)
